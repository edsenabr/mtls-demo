...
if [[ ! -e certs/server.pem ]]; then
  ...
  #sign the server csr
  openssl x509 -req -in certs/server.csr -CA certs/ca.pem -CAkey certs/ca.key -CAcreateserial -out certs/server.pem -days 365 -sha256 -extfile <(printf "subjectAltName=DNS:*.amazonaws.com,DNS:*.com")
fi

if [[ ! -e certs/client.pem ]]; then
